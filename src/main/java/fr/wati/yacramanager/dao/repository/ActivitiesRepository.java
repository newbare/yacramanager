...
public interface ActivitiesRepository extends JpaRepository<Activities, Long>, JpaSpecificationExecutor<Activities> {
    ...
    @Query(nativeQuery = true,
            value = "select * from ("
                    ...
                    + "union "
                    + "select act.* from project project,employe emp,company comp,activities act,employesprojects ep where emp.company_id=comp.id and act.entitytype='Project' and act.entityid=project.id and emp.id=ep.employeeid and ep.projectid=project.id and comp.id=:companyId "
                    + "union "
