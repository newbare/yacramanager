...
public class CompanyServiceImpl implements CompanyService {
    ...
    public <S extends Company> S save(S entity) {
        ...
        applicationEventPublisher
                .publishEvent(ActivityEvent.createWithSource(this).user().operation(activityOperation).onEntity(Company.class, save.getId()).dto(toCompanyDTO(save)));
        return save;
    }
    ...
    public void delete(Long id) {
        ...
        applicationEventPublisher.publishEvent(ActivityEvent.createWithSource(this).user().operation(ActivityOperation.DELETE).onEntity(Company.class, id));
    }
            ...
            case DATE_RANGE:
                ...
                if ("licenseEndDate".equals(filter.getField())) {
                    if (filterDate.isRangedDate()) {
                        ...
                        return Specifications.where(new Specification<Company>() {

                            @Override
                            public Predicate toPredicate(Root<Company> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
                                return cb.equal(root.<CompanyAccountInfo> get("companyAccountInfo").<LocalDate> get("expiredDate"), filterDate.getValue().getDate().toLocalDate());
                            }
                        });
                    }
                }
    ...
    public Specification<Company> getGlobalSpecification(String text) {
        return CommonSpecifications.likeIgnoreCase(text, Company_.name);
    }
}
